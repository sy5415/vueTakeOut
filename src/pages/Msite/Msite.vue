        ...
        <nav class="msite_nav">
          <div class="swiper-container">
            <div class="swiper-wrapper">
              <div class="swiper-slide" v-for="(category,index) in categorysArr2"  :key='index'>
                <div class="link_to_food" v-for="(cg,index) in category" :key='index'>
...
export default {
  ...
  mounted(){
    //分发异步action,蒋数据从后台请求到vuex中 
    this.$store.dispatch('getCategorys')
    this.$store.dispatch('getShops')
  },
  watch:{
    categorysArr2(){
      //保证下一次组件加载完毕时调用  这里是为了商品分类有下方小圆点因为用了swiper插件
      this.$nextTick(()=>{
